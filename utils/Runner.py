...
class BaseRunner(object):
    ...
    def predict(model: BaseModel,
        ...
        for step, batch in enumerate(test_loader):
            ...
            if return_sim:
                sims, last_types = BaseRunner.compute_query_last_sim(
                    model, batch_gpu)
    ...
    @staticmethod
    @torch.no_grad()
    def compute_query_last_sim(model: BaseModel,
        ...
        last_types = torch.zeros((batch_size, ),
                                 device=model.device)  # 0:none 1:rec 2:query

        for i in range(batch_size):
            valid_indices = torch.nonzero(valid_mask[i],
...
class SarRunner(BaseRunner):
    ...
    def train_epoch(self, epoch: int, model: BaseModel):
        ...
        src_flags = build_flags(loss_dict['src'])
        logging.info(
            "epoch:{:d} diagnostics |rec flags:{} {}| |src flags:{} {}|".
            format(epoch, rec_flags, rec_diag, src_flags, src_diag))
    ...
    def evaluate(self, model: BaseModel, mode: str):
        ...
        if src_sims is not None and len(src_sims) > 0:
            for last_type, type_key in [(2, 'last_query'), (1, 'last_rec')]:
                ...
                buckets = [
                    sims_group <= quantiles[0],
                    (sims_group > quantiles[0]) & (sims_group <= quantiles[1]),
                    sims_group > quantiles[1]
                ]
                ...
                for mask, bkey in zip(buckets, bucket_keys):
                    ...
                    results[f"src_{type_key}_{bkey}"] = utils.format_metric(
                        bucket_eval)
