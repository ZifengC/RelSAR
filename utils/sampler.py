...
class Sampler(object):
    def __init__(self, data_path, search, user_vocab) -> None:
        ...
        if {'rec_his', 'src_session_his'}.issubset(raw_data.columns):
            ...
            if rec_zero > 0 or src_zero > 0:
                print(f"[Sampler] {data_path} rec_his<=0 {rec_zero} 条, "
                      f"src_session_his<=0 {src_zero} 条, 将补至少1条非零历史")
            if rec_zero > 0 and not self.virtual_rec_items:
                print(f"[Sampler] {data_path} 无可用虚拟 rec 历史")
            if src_zero > 0 and not self.virtual_src_sessions:
                print(f"[Sampler] {data_path} 无可用虚拟 src 历史")
        self.data = raw_data
    ...
    def _build_virtual_rec_items(self, raw_data):
        ...
        return [int(i) for i in counts.index if int(i) > 0]

    def _build_virtual_src_sessions(self, user_vocab):
        counter = {}
        for _, info in user_vocab.items():
            for sid in info.get('src_session_his', []):
                sid = int(sid)
                if sid <= 0:
                    continue
                counter[sid] = counter.get(sid, 0) + 1
        return [sid for sid, _ in sorted(counter.items(), key=lambda x: -x[1])]
    ...
    def _get_virtual_items(self, source):
        if source == 'rec':
    ...
    def _ensure_min_one_nonzero(self, items, ts, max_len, source):
        if any(int(it) > 0 for it in items):
            return items, ts
        items = list(items)
        ts = list(ts)
        items[0] = self._get_virtual_items(source)
        ts[0] = -1.0
        if len(items) != max_len:
            raise ValueError(
                f"{source} history length mismatch: {len(items)}/{max_len}")
        return items, ts
