# Prompt: 将当前项目原地优化成 RelSAR

你是 coding agent。这个 README 会直接放在当前项目根目录中。请把**当前项目**原地优化成 RelSAR：基于当前项目的数据接口、训练流程和预测头，加入下面定义的轻量机制。目标不是从其他目录复制一个复杂版 RelSAR，而是在当前代码上做最小必要改动，得到一个更简洁、逻辑清楚的 RelSAR 实现。
...
Multi-Intent Discovery
 ...
 -> 当前项目 prediction heads
```

核心原则：

- 保留当前项目的数据接口、loss 接口、PLE prediction head、query-item alignment、history contrastive loss。
